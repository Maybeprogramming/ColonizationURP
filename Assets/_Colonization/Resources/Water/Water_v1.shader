        ...
        Pass
        {
            ...
            void SampleWaves(float2 uv, float time, out float3 normal, out float height)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    float steep2 = str * 0.3;

                    CalcWave(dir, freq, amp2, spd2, steep2, uv, time, slope, height);
            ...
            half4 frag(Varyings i) : SV_Target
            {
                ...
                float3 waterColor = lerp(_ColorShallow.rgb, _ColorDeep.rgb, depthFactor);
                ...
                float alpha = saturate(depthDiff * 2.0 + _DepthOffset);
                alpha = max(alpha, 0.1);
